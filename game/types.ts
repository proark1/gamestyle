...
export type Piece = { id: string; kind: Kind; x: number; y: number; z: number; rotation: number; vy: number; tilt: number; heldBy?: string; unstable: number };
...
export type World = { phase: 'lobby' | 'playing' | 'won' | 'lost'; mode: 'normal' | 'practice'; started: number; clock: number; water: number; pieces: Piece[]; players: Player[]; bestHeight: number; events: GameEvent[]; crane: { owner: string | null; piece: string | null; x: number; y: number; z: number }; seed: number };
...
export type Action = { type: 'start' | 'restart' | 'grab' | 'place' | 'rotate' | 'rescue' | 'crane' | 'crane-move' | 'crane-drop' | 'wave'; target?: string; x?: number; z?: number; y?: number; rotation?: number };
