...
export class GameScene {
  ...
  ghost:T.Group|null=null;ghostKind='';ghostSpot:{x:number;y:number;z:number;error:string|null}|null=null;
  ...
  constructor(public host:HTMLElement,public callbacks:Callbacks){
    ...
    this.renderer.shadowMap.enabled=!mobile;this.renderer.shadowMap.type=T.PCFSoftShadowMap;this.renderer.toneMapping=T.ACESFilmicToneMapping;this.renderer.toneMappingExposure=1.3;
    ...
    this.scene.add(island());this.water=new T.Mesh(new T.PlaneGeometry(250,250),new T.MeshStandardMaterial({color:'#73b5b1',roughness:.38,metalness:.05,transparent:true,opacity:.86}));this.water.rotation.x=-Math.PI/2;this.scene.add(this.water);
  ...
  setSnapshot(snapshot:Snapshot,id:string,local=false){
    ...
    if(player){
      if(!this.predicted||!previous||snapshot.world.started!==previous.world.started||local||Math.hypot(this.predicted.x-player.x,this.predicted.z-player.z)>1.8||player.down||player.rescued){this.predicted=structuredClone(player);this.jumpSeq=Math.max(this.jumpSeq,player.lastJump);}
      else {const p=this.predicted;p.x=T.MathUtils.lerp(p.x,player.x,.22);p.z=T.MathUtils.lerp(p.z,player.z,.22);if(Math.abs(p.y-player.y)>1.2){p.y=player.y;p.vy=player.vy;p.grounded=player.grounded;}p.down=player.down;p.breath=player.breath;}
    }
    this.projectionDirty=true;
    for(const p of snapshot.world.pieces)if(!this.pieces.has(p.id)){const m=junk(p.kind);m.userData.pieceId=p.id;m.traverse(o=>o.userData.pieceId=p.id);this.pieces.set(p.id,m);m.position.set(p.x,p.y,p.z);this.scene.add(m);}
  ...
  pointerDown=(e:PointerEvent)=>{if(this.menu||this.paused)return;this.host.setPointerCapture(e.pointerId);this.drag={id:e.pointerId,x:e.clientX,y:e.clientY,moved:false};};
  ...
  pointerUp=(e:PointerEvent)=>{
    if(this.drag&&!this.drag.moved){this.pointerMove(e);const hit=this.pick();if(hit){this.selected=hit.id;if(this.world?.world.pieces.some(p=>p.heldBy===this.localId))this.interact();}}
    this.drag=null;if(this.host.hasPointerCapture(e.pointerId))this.host.releasePointerCapture(e.pointerId);
  };
  ...
  interact(){
    ...
    if(this.world.world.pieces.some(p=>p.heldBy===this.localId)){
      this.updateGhost();if(this.ghostSpot)this.callbacks.action({type:'place',x:this.ghostSpot.x,z:this.ghostSpot.z});
  ...
  updateGhost(){
    ...
    if(this.ghostKind!==held.kind){if(this.ghost){this.ghost.removeFromParent();this.disposeObject(this.ghost,true);}this.ghost=junk(held.kind);this.ghost.traverse(o=>{if(o instanceof T.Mesh){o.material=(o.material as T.MeshStandardMaterial).clone();Object.assign(o.material,{transparent:true,opacity:.38,depthWrite:false});o.castShadow=false;}});this.scene.add(this.ghost);this.ghostKind=held.kind;}
    let x=this.predicted.x+Math.sin(this.predicted.angle)*2.9,z=this.predicted.z+Math.cos(this.predicted.angle)*2.9;
    if(this.pointerActive&&!this.drag){const hit=this.pick();if(hit){x=hit.point.x;z=hit.point.z;}else{this.ray.setFromCamera(this.pointer,this.camera);const v=new T.Vector3();if(this.ray.ray.intersectPlane(new T.Plane(new T.Vector3(0,1,0),-this.predicted.y),v)){x=v.x;z=v.z;}}}
    this.ghostSpot=placement(this.world.world,this.predicted,held,x,z);const spot=this.ghostSpot;this.ghost!.visible=true;this.ghost!.position.set(spot.x,spot.y,spot.z);this.ghost!.rotation.y=held.rotation*Math.PI/2;
    this.ghost!.traverse(o=>{if(o instanceof T.Mesh)(o.material as T.MeshStandardMaterial).color.set(spot.error?'#de6b53':'#91cf8a');});
  }
  ...
  frame=(now:number)=>{
    ...
    if(this.world&&this.predicted){
      ...
      for(const p of world.pieces){const m=this.pieces.get(p.id);if(!m)continue;let x=p.x,y=p.y,z=p.z;if(p.heldBy===this.localId){x=this.predicted.x;y=this.predicted.y+2.05;z=this.predicted.z;}m.position.lerp(new T.Vector3(x,y,z),1-Math.exp(-dt*18));m.rotation.y=p.rotation*Math.PI/2;m.rotation.z=p.tilt;}
      ...
      this.updateGhost();const item=this.currentTarget();this.ring.visible=!!item&&!world.pieces.some(p=>p.heldBy===this.localId);
      if(item)this.ring.position.set(item.x,item.y+dimensions(item).h+.03,item.z);
    ...
    this.updateCamera(dt);this.renderer.render(this.scene,this.camera);this.frameId=requestAnimationFrame(this.frame);
  };
  disposeObject(object:T.Object3D,materials=false){object.traverse(o=>{if(o instanceof T.Mesh)o.geometry.dispose();if(o instanceof T.Sprite){o.material.map?.dispose();o.material.dispose();}if(materials&&o instanceof T.Mesh){for(const m of Array.isArray(o.material)?o.material:[o.material])m.dispose();}});}
