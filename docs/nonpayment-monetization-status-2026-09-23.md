...
3. Expand the four-week challenge rotation beyond Stack or Sink when another game has a verifier. The paid challenge lane needs a verified paid game and remains unavailable while paid admission is dormant. Schedule the supplied `ranked:finalize` command in the deployment environment so prizes do not wait for a leaderboard read.
