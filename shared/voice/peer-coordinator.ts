...
export async function handleVoicePeer(
  ...
  for (let attempt = 0; attempt < 16; attempt++) {
    ...
    state.members = state.members.filter(
      // Loading a new 3D game can briefly stall the party page's heartbeat.
      // Keep its voice seat for the same reconnect grace as the party itself.
      (m) =>
        allowed.has(m.id) &&
        now - m.seen < (session.game === 'party' ? 60000 : 15000),
    );
