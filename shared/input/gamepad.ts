...
export function bindGamepad() {
  ...
  const poll = (time: number) => {
    ...
    // In a party, only the active surface consumes controller buttons. The
    // parent handles its dialogs; otherwise the embedded game handles play.
    let parentDialog = false;
    try {
      parentDialog =
        window.parent !== window &&
        !!window.parent.document.querySelector('.party-playing') &&
        !!window.parent.document.querySelector(
          '[role="dialog"][aria-modal="true"]',
        );
    } catch {
      /* Only same-origin party frames share controls. */
    }
    if (
      parentDialog ||
      (document.querySelector('iframe.party-game-frame') &&
