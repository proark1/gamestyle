...
export function usePeerRoom<S extends RoomSnapshot>(options: Options<S>) {
  ...
  useEffect(() => {
    if (status !== 'expired' || !inPartyMode() || window.parent === window)
      return;
    window.parent.postMessage(
      {
        type: 'party-game-error',
        code: new URLSearchParams(location.search).get('party')?.toUpperCase(),
      },
      location.origin,
    );
  }, [status]);
  ...
  async function copyInvite() {
    ...
    try {
      await navigator.clipboard.writeText(
        inPartyMode()
          ? `${publicGameOrigin()}/party?room=${new URLSearchParams(location.search).get('party')}`
