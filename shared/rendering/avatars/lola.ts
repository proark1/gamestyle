...
function grooves(side: number) {
  const tie = TIE.clone()
    .multiply(new T.Vector3(side, 1, 1))
    .sub(CAP.centre);
  const end = tie.divide(CAP.radii).normalize();
 ...
 * (`outfit`) and rig are the kid's; see `hoopKid`.
 */
export function lola(color = 0, outfit: WorkerOutfit = {}) {
  const kid = hoopKid(color, outfit, { teeth: false, ears: false });
  const { head } = kid;
  ...
  for (const [i, [x, y, roll]] of bangs.entries()) {
    const bang = lock(head, [0.115, 0.062, 0.036]);
    onFace(bang, x, y, 0.012, [SKULL]);
    bang.rotateZ(roll);
  ...
  strokes(head, [...grooves(-1), ...grooves(1)], 0.0045, GROOVE);
