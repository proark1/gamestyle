...
export function openSqlite(filename = databasePath()) {
  ...
  database.exec('PRAGMA journal_mode = WAL; PRAGMA busy_timeout = 5000; PRAGMA synchronous = FULL;');
  return database;
}
...
export function migrateSqlite(database, directory = resolve('drizzle')) {
  database.exec('CREATE TABLE IF NOT EXISTS _game_migrations (name TEXT PRIMARY KEY NOT NULL, applied INTEGER NOT NULL)');
  for (const name of readdirSync(directory).filter(name => /^\d+.*\.sql$/.test(name)).sort()) {
    database.exec('BEGIN IMMEDIATE');
    try {
      if (!database.prepare('SELECT name FROM _game_migrations WHERE name = ?').get(name)) {
        database.exec(readFileSync(resolve(directory, name), 'utf8'));
        database.prepare('INSERT INTO _game_migrations (name, applied) VALUES (?, ?)').run(name, Date.now());
      }
