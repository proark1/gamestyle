...
void test('one shared assignment survives races and reloads; real outcomes, revocation and persistent voice', async () => {
  ...
  const guestPass = { id: guest.playerId, token: guest.token };
  await toggleReady(store, code, guestPass, true, 1000);
  await startPartyTournament(store, code, pass, 1000);
  await partyPlayerAction(store, code, pass, 'briefing_ready', 0, 6000);
  await partyPlayerAction(store, code, guestPass, 'briefing_ready', 0, 6000);
  ...
  assert.equal(
    recovered.view.voiceRecovered,
    false,
    'party voice retains its seat during a short loading interruption',
  );
