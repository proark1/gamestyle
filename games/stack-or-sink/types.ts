...
export const ITEMS: Record<
  Kind,
  { name: string; w: number; h: number; d: number; color: string }
> = {
  crate: { name: 'Wooden crate', w: 1.6, h: 1.3, d: 1.6, color: '#b58a55' },
  pallet: { name: 'Pallet', w: 2.4, h: 0.45, d: 2, color: '#cda66b' },
  sofa: { name: 'Old sofa', w: 2.8, h: 1.05, d: 1.25, color: '#d59940' },
  bathtub: { name: 'Bathtub', w: 2.5, h: 0.85, d: 1.3, color: '#d9e3db' },
  fridge: { name: 'Fridge', w: 1.4, h: 1.8, d: 1.4, color: '#9dbcb4' },
  plank: { name: 'Long plank', w: 3.6, h: 0.25, d: 1.2, color: '#bb9762' },
};
export type Piece = {
  id: string;
  kind: Kind;
...
export function dimensions(p: Pick<Piece, 'kind' | 'rotation'>) {
  const item = ITEMS[p.kind];
  return p.rotation % 2 ? { w: item.d, h: item.h, d: item.w } : item;
}
