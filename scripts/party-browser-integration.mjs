...
const server = createServer(async (req, res) => {
  ...
  try {
    if (req.url.startsWith('/api/')) {
      ...
      if (req.url === '/api/peer') peerActor = b.id;
      let result;
      if (req.url === '/api/peer') {
        ...
        if (
          ...
          result = {
            state: await party.partyPlayerAction(
              store,
              b.code,
    ...
    const root = resolve(process.env.PARTY_CLIENT_DIR ?? 'dist/native'),
    ...
    if (
      ...
      const state = JSON.parse(rows.get('party:' + code)?.state ?? '{}');
      if (
        peerActor &&
        error.status === 401 &&
        error.message === 'This party round has ended. Return to the party.' &&
        (['intermission', 'finished'].includes(state.status) ||
          state.reports?.[peerActor] !== undefined)
      )
        closedRequests++;
...
try {
  ...
  for (const [i, p] of [host, guest].entries()) {
    ...
    if (process.env.SMOKE_LOW_GRAPHICS === '1')
  ...
  for (const page of pages)
    await page
      .getByRole('button', { name: 'Got it — I’m ready', exact: true })
      .click({ timeout: 15000 });
  for (const page of pages)
  ...
  for (const page of pages) {
    ...
    await page.evaluate(() => {
      window.__partyDebug = { messages: [], keys: [] };
      window.addEventListener('message', (event) => {
        if (event.data?.type?.startsWith('party-'))
          window.__partyDebug.messages = [
            ...window.__partyDebug.messages.slice(-8),
            { type: event.data.type, key: event.data.keyCode },
          ];
      });
      window.addEventListener(
        'keydown',
        (event) => window.__partyDebug.keys.push(event.code),
        true,
      );
    });
    await frame.evaluate(() => {
      window.__partyDebug = { keys: [] };
      window.addEventListener(
        'keydown',
        (event) => window.__partyDebug.keys.push(event.code),
        true,
        ...
        console.log(
          JSON.stringify({
            ...
            parent: await page.evaluate(() => ({
              debug: window.__partyDebug,
              focus: document.activeElement?.tagName,
              voice: document.querySelector('.voice-trigger')?.outerHTML,
  ...
  await pages[0]
    ...
    .catch(async (error) => {
      console.log(
        JSON.stringify({
          phase: 'reload',
          party: await party.getPartyRoom(store, code),
          parent: await pages[0].locator('body').innerText(),
          frames: await Promise.all(
            pages[0].frames().map(async (f) => ({
              url: f.url(),
              text: await f
                .locator('body')
                .innerText()
  ...
  for (const page of pages) {
    await page.getByRole('button', { name: 'Party menu', exact: true }).click();
    await page
      .getByRole('button', { name: 'Give up round…', exact: true })
      .click();
    await page
      .getByRole('button', { name: 'Give up · 0 points', exact: true })
      .click();
  }
  for (const page of pages) {
  ...
  console.log(
    game +
      `: PASS: two real browsers share a party round, bridge push-to-talk and preserve voice into intermission (${checkpointConflicts} recoverable checkpoint conflicts; ${closedRequests} expected requests after forfeit).`,
  );
} finally {
  await browser.close();
