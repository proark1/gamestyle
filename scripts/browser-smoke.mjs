...
try {
  for (const game of games) {
    ...
    const practice = page
      .getByRole('button', {
        name: /practice|solo|alone|training|üben|alleine|start match|let.s shop|let.s bounce|start game|start (the )?shift|start round|ready.*play|vs bots|learn with|gloves up|lock in .*fighter/i,
      })
      .first();
